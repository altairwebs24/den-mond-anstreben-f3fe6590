...
const productSchema = z.object({
  ...
  category: z.string().min(2).max(80), collection_id: z.string().uuid().nullable(), price: z.number().min(0).nullable(),
...
const requireAdmin = async (context: { supabase: SupabaseClient<Database>; userId: string; claims: Record<string, unknown> }) => {
  ...
  if (!role && adminEmails.has(email)) {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const result = await supabaseAdmin.from("user_roles").upsert({ user_id: context.userId, role: "admin" }, { onConflict: "user_id,role" }).select("id").single();
    role = result.data;
  }
...
export const listCollections = createServerFn({ method: "GET" }).handler(async () => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data, error } = await supabaseAdmin.from("collections").select("*").eq("published", true).order("name");
  if (error) throw new Error("Could not load collections.");
  return data;
});

export const listAdminCollections = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(async ({ context }) => {
  await requireAdmin(context);
...
export const saveCollection = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => z.object({ id: z.string().uuid().optional(), name: z.string().trim().min(2).max(80), slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), published: z.boolean() }).parse(input)).handler(async ({ data, context }) => {
  await requireAdmin(context);
  const { id, ...values } = data;
  const result = id ? await context.supabase.from("collections").update(values).eq("id", id).select().single() : await context.supabase.from("collections").insert(values).select().single();
  if (result.error) throw new Error(result.error.message);
  return result.data;
});

export const deleteCollection = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => z.object({ id: z.string().uuid() }).parse(input)).handler(async ({ data, context }) => {
  await requireAdmin(context);
